...
testada). **108 testes verdes** (98 + 6 do #24 + 3 do fix `relatedFiles`/fallthrough + 1 do fix lock-só-com-trigger), lint+build limpos.
...
- **2026-05-25 (#24 — fix countdown loop: lock só com `trigger` real):**
  smoke do user: o countdown disparava em loop mesmo sem ter salvo nada. Causa:
  `VitestAdapter.watch` chama `api.rerunFiles()` (= `[ all ]` / auto-fire do
  countdown) e o watcher do Vitest re-emite `onWatcherRerun(files, undefined)`
  — `relatedFiles` populado, `trigger` ausente. A store preferia `relatedFiles`,
  re-locava, e o `wireCountdown` re-disparava → triggerAll → re-lock, infinito.
  Fix em `src/tui/store.ts` no caso `rerun`: amarrar o lock ao `trigger` (=
  save real); sem trigger ⇒ `lockedFiles=undefined` (manual `[ all ]`/`[ failed ]`
  / auto-fire continuam funcionando porque só `command` + `WatchHandle` rodam).
  TDD-lite red→green: +1 teste em `tui-store.test.ts` ("rerun with relatedFiles
  but no trigger does NOT lock"). **108 verdes**, lint+build limpos. Contrato
  do `check` **byte-inalterado** (mudança TUI-only).
